...
@section('title', 'Your Pokémon')
@section('title_nav')
<a href="{{ url('/') }}" class="flex items-center hover:opacity-70 transition-all">
    ...
    <span class="font-semibold text-xl tracking-tight ml-5">Pokémon</span>
</a>
@endsection
@section('content')
        ...
        <div class="flex flex-col justify-between p-4 leading-normal w-2/3">
            ...
            <div class="absolute bottom-4 right-5">
                <span class="inline-flex mt-2 -space-x-px overflow-hidden rounded-md border border-gray-900 shadow-sm">
                    <a href="{{ url('pokemon/'.$entity->id.'/edit') }}" class="transition-all inline-block px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-600 hover:text-white focus:relative">Edit</a>

                    <form action="{{ url('pokemon/' . $entity->id) }}" method="POST" class="transition-all inline-block text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white focus:relative">
                        @csrf
                        @method('DELETE')
...
@else
<h1 class="w-full p-10 text-2xl font-bold text-center">You need to be logged to view the Pokémon.</h1>
@endcan
